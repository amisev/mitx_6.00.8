...
class norm:
    ...
    def pdf(self, x):
        d = len(self.mu)
        if d == 1:
            return 1/((np.pi*2)**(1/2)*self.sigma)*np.exp(-1/2*(x-self.mu)**2/self.sigma**2)
        else:
            return 1/((np.pi*2)**(d/2)*np.linalg.det(self.sigma)**(1/2))*np.exp(-1/2*np.dot(np.dot((x-self.mu).T, np.linalg.inv(self.sigma)), (x-self.mu)))
...
def compute_empirical_distribution(values):
    ...
    # YOUR CODE HERE
    #
    distribution = norm(mu = np.mean(values), sigma = np.cov(values, rowvar=0)).pdf
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def compute_empirical_mutual_info_nats(var1_values, var2_values):
    ...
    for k1 in p1:
        for k2 in p2:
            empirical_mutual_info_nats += p12[(k1, k2)]*np.log(p12[(k1, k2)]/(p1[k1]*p2[k2]))
